#!/usr/bin/env python3
"""toolsets 系统 —— my-hermes 照搬版（保留后续可移植性）。

本文件从原版 hermes-agent/toolsets.py（1030 行）**原样照搬**，未做精简：
- 工具集定义（TOOLSETS）含原版全部叶子/组合/平台工具集，即使 my-hermes
  尚未移植对应工具（如 web/browser/hermes-telegram 等）——resolve 出的
  未注册工具名会被 registry.get_definitions 自动忽略，行为安全；
- gateway.platform_registry 等未移植依赖被原版 try/except 兜底，照搬后
  在 my-hermes 中对应分支静默跳过；
- 保留 __main__ demo，便于对照原版行为。

直接照搬而非精简的理由：后续继续移植原版工具/平台时，本文件无需再改，
与原版 diff 最小，可移植性最好。
"""
